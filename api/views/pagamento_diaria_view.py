    ...
    def post(self, request, diaria_id, format=None):
        ...
        if serializer_pagamento.is_valid():
            ...
            if diaria.status == 1:
                ...
                # realizar pagamento
                return Response({"Diaria paga com sucesso"}, status=status_http.HTTP_200_OK)
            return Response({"Não é possível pagar essa diaria"}, status_http.HTTP_400_BAD_REQUEST)
