...
def relacionar_candidata_diaria(diaria_id, diarista_id):
    ...
    diarista = Usuario.object.get(id=diarista_id)
    ...
    if diaria.candidatas__count >= 3:
        raise serializers.ValidationError("A diaria já possui 3 candidatas")
    if diaria.candidatas__count < 2:
        diaria.candidatas.add(diarista)
    if diaria.candidatas__count == 2:
        diaria.candidatas.add(diarista)
        #selecionar_diarista_diaria(diaria_id)
        
def contratar_diarista_diaria(diaria, diarista_id):
    if diaria.status != 2:
        raise serializers.ValidationError("Apenas diáris pagas podem ser realizadas")
    diarista = Usuario.object.get(id=diarista_id)
